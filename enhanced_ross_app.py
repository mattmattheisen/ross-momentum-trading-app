"""
Ross-Style Momentum Signal Generator
===================================
Professional momentum trading signals with full market scanning.
...
# ------------- ALPACA MARKET SCANNING -------------
def full_market_gap_scan(scan_date: date, cfg: dict) -> pd.DataFrame:
    """Scan entire market for gap opportunities using Alpaca"""
    if not ALPACA_AVAILABLE:
        st.error("Alpaca not available for full market scan.")
        return pd.DataFrame()
    ...
    if not key or not secret:
        st.error("Alpaca API credentials not found. Add them to your Streamlit secrets.")
        return pd.DataFrame()
    
    # Get all active symbols
    symbols = get_active_symbols()
    if not symbols:
        st.error("Could not retrieve symbol list from Alpaca.")
        return pd.DataFrame()
    
    st.info(f"Scanning {len(symbols):,} symbols from NYSE/NASDAQ/AMEX...")
    
    rows = []
    total_batches = len(list(chunk_list(symbols, 100)))
    ...
    for batch in chunk_list(symbols, 100):
        batch_count += 1
        progress_bar.progress(batch_count / total_batches)
        status_text.text(f"Processing batch {batch_count}/{total_batches} ({len(batch)} symbols)...")
        
        for symbol in batch:
            try:
                # Get daily data for this symbol
                daily_data = get_daily_data(symbol, period="10d")
                ...
                prev_data = daily_data.iloc[-2]
                
                # Calculate gap
                gap_pct = (today_data['open'] - prev_data['close']) / prev_data['close']
                
                # Apply filters
                if not (cfg['price_min'] <= today_data['open'] <= cfg['price_max']):
                    continue
                ...
                # Calculate RVOL
                rvol = calculate_rvol(symbol, today_data['volume'])
                if rvol < cfg['rvol_min']:
                    continue
                
                # Calculate opportunity score
                score = calculate_opportunity_score(gap_pct, rvol, today_data['open'])
                
                rows.append({
                    "Symbol": symbol,
                    "Price": f"${today_data['open']:.2f}",
                    "Gap %": f"{gap_pct:.1%}",
                    "RVOL": f"{rvol:.1f}",
                    "Volume": f"{int(today_data['volume']):,}",
                    "Score": score
                })
    ...
    if rows:
        ...
        return df_scan.sort_values("Score", ascending=False)
    else:
        return pd.DataFrame()

def calculate_opportunity_score(gap_pct: float, rvol: float, price: float) -> int:
...
def scan_for_opportunities(symbols: List[str], cfg: dict) -> pd.DataFrame:
    ...
    for i, symbol in enumerate(symbols):
        ...
        try:
            ...
            if rvol >= cfg['rvol_min']:
                score = calculate_opportunity_score(gap_pct, rvol, current_price)
...
def main():
    ...
    if 'market_scan_results' in st.session_state and not st.session_state.market_scan_results.empty:
        ...
        top_market_symbols = st.session_state.market_scan_results.head(15)['Symbol'].tolist()
    ...
    with tab1:
        ...
        with col2:
            ...
            if ALPACA_AVAILABLE:
                ...
                if key and secret:
                    if st.button("🌍 Scan Entire Market", type="secondary"):
                        with st.spinner("Scanning entire market... may take 30-60 seconds"):
                            df_scan = full_market_gap_scan(trade_date, cfg)
                ...
                st.warning("Alpaca libraries not installed. Full market scanning not available.")
    ...
    with tab5:
        ...
        # Full market scanning info
        st.write("### 🌍 Full Market Scanning")
        
        st.info("""
        **Professional Market Screening:**
        - Scans 8,000+ NYSE/NASDAQ/AMEX stocks automatically
        - Real-time gap detection with volume confirmation
        - Opportunity scoring and ranking system
        - Discovers momentum plays outside your watchlist
        
        **Requirements:**
        - Free Alpaca account (alpaca.markets)
        - API keys added to Streamlit secrets
        - 30-60 seconds scanning time
        
        **Benefits over manual screening:**
        - Never miss opportunities in unknown stocks
        - Professional-grade institutional tools
        - Time-saving automated research
        ...
        **🔧 TECHNICAL NOTES:**
        ...
        - Alpaca integration provides professional market scanning
        - Real-time data requires premium data feeds for live trading
        ...
        st.write("""
        **Version:** 2.0 - Full Market Scanning Release
        **Data Sources:** Yahoo Finance (Free) + Alpaca Markets (Professional)
        **Update Frequency:** Real-time for display, cached for performance
        **Deployment:** Streamlit Cloud
        
        **Features:**
        - Real-time market scanning (custom lists + full market)
        - Multiple momentum trading strategies
        - Professional market screening tools
        ...
        **Recommended Usage:**
        1. Run full market scan before market open (8:30 AM)
